    ...
    var tagit = {
        "addTag": function(tag) {
            ...
            if (!$(element).parent().length) {
                var lastTag = $('.btn-tag', self).last();
                if (lastTag.length === 0)
                    element.insertBefore($('.tagit-edit-handle', self));
                else
                    element.insertAfter(lastTag);
            }
        ...
        tagit: function() {
            ...
            return this.each(function() {
                ...
                if (e.is(".tagit")) {
                    ...
                    var editHandle = $('<li class="tagit-edit-handle"></li>');
                    var input = $('<input type="text" class="no-style" />');
                    var autocomplete = $("<ul></ul>").addClass('dropdown-menu');
                    ...
                    e.bind("focusin", function(event) {
                        ...
                        if (!focusing) {
                            ...
                            //window.setTimeout(function () { autocomplete.removeClass("open") }, 1);
                            input.val("");
                        }
                    });
                    ...
                    input.keypress(function(event) {
                        ...
                        if (tag) {
                            ...
                            $.each(availableTags, function(i, e) {
                                if (e.toLowerCase().indexOf(tag) != -1) {
                    ...
                    editHandle.append(input).append(autocomplete);

                    var lastTag = $('li', e).last();
                    if (lastTag.length === 0)
                        e.append(editHandle);
                    else
                        editHandle.insertAfter(lastTag);
